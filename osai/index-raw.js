...
const $on = (element, event, listener, option = false) => {
    try {
        let addListener = (listenerElement, index) => {
            let listenerFn = (e, element) => listener(e, element, index);
            if (option === "on" || option?.on) listenerElement["on" + event] = listenerFn; else if (option === "remove" || option === "del") listenerElement.removeEventListener(event, listenerFn, false); else {
