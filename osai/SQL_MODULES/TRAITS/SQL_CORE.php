...
/**
 * Trait SQL_CORE
 * SQL Class CRUD Operation and it's required functions
...
trait SQL_CORE {
    ...
    final public function get_join(array $query_array,...$opt):?array{return$this->query_select($query_array,null,$opt);}
    ...
    final public function del(string $table,string $clause,int $debug=0):bool{return$this->query_delete($table,$clause,$debug);}
    /// DB Selectors Full
    public string $query;
    /**
     * INSERT
     * @param string $table table to insert into
     * @param mixed ...$option args = "columns" || "column=value" must be first value;
     * other args =  "values", "MODE", DEBUG;
     * {string} MODE = 'combo' [default] || 'multi' || 'multi2';
     * [combo] constructing INSERT like "UPDATE" I.E [column_name=column_value]
     * [multi] constructing INSERT with one column row multiple value rows I.E [(column) VALUES ($value), ($value), ...]
    ...
    final public function query_insert(string $table, ...$option) : ?bool {
        ...
        elseif($mode == "multi")
            ...
            if(trim(substr($values,0)) === "(") $mode = "multi2";
            $insert_clause = implode(",", array_map(fn($v) => "`$v`", explode(",", str_replace("`","", $insert_clause))));
            $values = rtrim($values,",");
            $values = $mode == "multi2" ? "$values" : "($values)";
            $q = "INSERT INTO $table ($insert_clause) VALUES $values";
        }
        
        return $this->query($q, "insert", $debug);
    }
    /**
     * SELECT
    ...
    final public function query_select($columns, ...$option) : ?array {
        ...
        if(is_array($columns)) {
            ...
            foreach ($columns['join'] as $col_opt) {
                $join = strtolower($col_opt['type'] ?? "");
                switch($join){
                    case "left": case "inner":case "right":
            ...
            $clause = $columns['clause'] ?? $option[1];
    ...
    final public function query_count(string $column, ...$option) : int {
        ...
        if(!is_int($option[1]))
            $clause = $option[1];
        return $this->query("SELECT COUNT($column) FROM $table $clause", "count",$debug);
    }
    /**
     * UPDATE
     * @param string $table table to update
     * @param string|array $update_values values to update, if {array} the function assumes multiple update mode
     * #ARRAY SHAPE
     * [
     ...
     * @example SQL_CORE::query_update($table,[
    ...
    final public function query_update(string $table, $update_values, ?string $clause = null, ...$misc) : bool {
        ...
        if(is_array($update_values)) {
            ...
            $update_values = @$update_values['values'] . rtrim($case_value, ",");
            $clause = rtrim($clause," AND");
        }

        return $this->query("UPDATE $table SET $update_values $clause", "update",$debug);
    }
    /**
     * DELETE
     * @param string $table table to insert into
     * @param string $clause criteria to follow to ensure delete
     * @param int $debug to display query before execution, for troubleshooting
     * @return bool returns true on execution
     **/
    final public function query_delete(string $table, string $clause, int $debug=0) : bool {
        return $this->query("DELETE FROM $table WHERE $clause", "delete", $debug);
    }
    /**
     * Query Engine
     * @param string $query
    ...
    final public function query(string $query, ...$option) {
        ...
        else $query_type = $option['type'] ?? $option['src'];
        ///LOOP AND FETCH AS
        ...
        if(mysqli_affected_rows(self::$link) == 0) return false;
        return $exec;
    }
     ...
     * @param string $except columns to exempt from the returning result, should be separated by comma (,)
     * @param null|Closure $fun a function that should execute at the end of a given row storage
